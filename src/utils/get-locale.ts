/** Script from https://github.com/GaloyMoney/galoy-mobile/blob/main/app/utils/locale-detector.ts */
...
export const matchOsLocaleToSupportedLocale = (
  ...
  const languageTagsFromOs = localesFromOs.map(
    osLocale => osLocale.languageTag
  );
  ...
  const languageCodeFromOs = localesFromOs.map(
    osLocale => osLocale.languageCode
  );
