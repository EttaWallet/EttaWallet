...
import { deviceIsIos14OrNewer } from '../utils/IosVersionUtils';
  ...
  return class WithPasteAware extends React.Component<P & PasteAwareProps> {
    ...
    checkClipboardContents = async () => {
      try {
        ...
        if (deviceIsIos14OrNewer()) {
          ...
          this.setState({
            isPasteIconVisible: clipboardHasContent,
            clipboardContent: null,
          });
          return;
        }
        ...
        if (
          ...
          !(
            value &&
            clipboardContent.toLowerCase().includes(value.toLowerCase())
          ) &&
    ...
    onPressPaste = async () => {
      ...
      const clipboardContent =
        storedClipboardContent || (await Clipboard.getString());
      if (!clipboardContent) {
        Logger.warn(
          'Attempted to paste but clipboard content empty. Should never happen.'
        );
        return;
      }
      this._isMounted
