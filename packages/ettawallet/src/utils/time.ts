...
function timeDifference(currTime: number, prevTime: number) {
  return (
    1.0 * (millisecondsSinceEpoch(currTime) - millisecondsSinceEpoch(prevTime))
  );
}
...
function millisecondsSinceEpoch(timestamp: number) {
  return Math.abs(differenceInYears(timestamp, Date.now())) > 40
    ? timestamp * 1000
    : timestamp;
}
...
export function getNextMondayAt8pmUtc() {
  ...
  return (
    toUtcDate(mondayDate).getTime() +
    20 * ONE_HOUR_IN_MILLIS +
    7 * ONE_DAY_IN_MILLIS
  );
}
